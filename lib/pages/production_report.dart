...
/// 2. Gráfico de líneas con cantidad producida por mes del año seleccionado
///    calculado sumando la cantidad de todos los lotes de ese mes
...
class ProductionReportPage extends StatefulWidget {
  ...
  @override
  State<ProductionReportPage> createState() => _ProductionReportPage();
}

class _ProductionReportPage extends State<ProductionReportPage> {
  /// Lista de lotes cargados desde el backend
  /// Cada elemento es un Map con los campos del LoteDTO
  ...
  void initState() {
    super.initState();
    /// Carga todos los lotes al iniciar la pantalla
    _cargarLotes();
  }

  // CARGA DE DATOS DESDE EL BACKEND
  /// Llama al LotService para obtener todos los lotes
  /// Tras cargar selecciona automáticamente el año más reciente con datos
  Future<void> _cargarLotes() async {
    ...
    try {
      ...
      setState(() {
        lotes = List<Map<String, dynamic>>.from(datos);

        /// Selecciona el año más reciente con datos
      ...
      setState(() {
        _error = 'Error al cargar los datos. Comprueba la conexión.';
        _cargando = false;
      });
    }
  }
  ...
  /// Extrae el año de la fechaElaboracion de cada lote y elimina duplicados
  List<int> get _aniosDisponibles {
    final anios = lotes
        .map((l) {
          try {
            return DateTime.parse(l['fechaElaboracion'] ?? '').year;
          } catch (e) {
            return null;
          }
        })
  ...
  /// Calcula los totales de cantidad producida por mes para el año seleccionado
  /// Agrupa los lotes por mes y suma el campo 'cantidad' de cada uno
  /// Devuelve una lista de 12 valores (uno por mes, 0 si no hay lotes)
  List<double> get _datosActuales {
    final totalesPorMes = List<double>.filled(12, 0);

    for (final lote in lotes) {
      try {
        final fecha = DateTime.parse(lote['fechaElaboracion'] ?? '');
        if (fecha.year == _anioSeleccionado) {
          /// Suma la cantidad del lote al mes correspondiente
          totalesPorMes[fecha.month - 1] +=
              (lote['cantidad'] as num?)?.toDouble() ?? 0;
        }
      } catch (e) {
        continue;
      }
                          ...
                          child: Material(
                            ...
                            child: Container(
                              ...
                              child: _datosActuales.every((v) => v == 0)
                                  ...
                                  ? const Center(
                                      child: Text(
                                        'No hay datos de producción para este año',
                                        style: TextStyle(
                                          color: Color(0xFF4A3B2A),
                                          fontSize: 14,
                                        ),
                                        ...
                                        Padding(
                                          ...
                                          child: Row(
                                            ...
                                            children: [
                                              ...
                                              /// Muestra el total o "—" si no hay producción ese mes
                                              Text(
                                                totalMes > 0
                                                    ? _formatearNumero(
                                                        totalMes.toInt())
                                  ...
                                  Container(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        Text(
                                          _formatearNumero(_datosActuales
                                              .fold(0.0,
                                                  (a, b) => a + b)
                                              .toInt()),
                                          style: const TextStyle(
  ...
  LineChartData _buildLineChartData() {
    ...
    final intervalo =
        (maxValor / 5).clamp(1.0, double.infinity).ceilToDouble();
            ...
            getTitlesWidget: (value, meta) {
              if (value == 0) {
                return const Text(
                  '0',
                  style: TextStyle(
                      fontSize: 10, color: Color(0xFF4A3B2A)),
                );
              }
              return Text(
      ...
      lineBarsData: [
        LineChartBarData(
          ...
          belowBarData: BarAreaData(
            show: true,
            color:
                const Color(0xFF4A3B2A).withValues(alpha: 0.08),
          ),
        ),
      ],
  ...
  /// Solo muestra los años que tienen lotes en la base de datos
          ...
          child: Row(
            ...
            children: [
              ...
              _buildNavItem(
                ...
                isSelected: true, // Producción es sección de Informes → activo
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
